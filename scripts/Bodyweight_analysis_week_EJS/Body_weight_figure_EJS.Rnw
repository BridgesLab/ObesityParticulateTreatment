...
#fit.lme.MCP230.indiv <- lmer(Weight~Week + Treatment + (1|Mouse), data=sub_data, REML=T)
#fit.lme.no.treatment.indiv <- lmer(Weight~Week + (1|Mouse)+(1+Treatment|Mouse), data=sub_data, REML=T)
